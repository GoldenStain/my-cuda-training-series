...
// 数值比较（容忍FP32精度误差）
bool allclose(const float* a, const float* b, int N, float rtol=1e-4, float atol=1e-5) {
    for (int i = 0; i < N; ++i) {
...
int main() {
    ...
    // 测试用例1：全零输入（简单验证）
    for (int i = 0; i < N; ++i) host_x[i] = 0.0f;
    ...
    // GPU计算
    CUDA_CHECK(cudaMemcpy(device_x, host_x, N * sizeof(float), cudaMemcpyHostToDevice));
    
    dim3 grid( (N + NUM_THREADS - 1) / NUM_THREADS );
    dim3 block(NUM_THREADS);
    softmax_fp32_per_token_kernel<NUM_THREADS><<<grid, block>>>(device_x, device_y, N);
    
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaGetLastError()); // 捕获kernel错误
    
    CUDA_CHECK(cudaMemcpy(host_y_gpu, device_y, N * sizeof(float), cudaMemcpyDeviceToHost));
